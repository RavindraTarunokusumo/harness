...
Rules:
- At the start of every session, from the current checkout, run `python scripts/ensure_gitnexus.py`. A linked worktree is a different registry path than the primary checkout; without this, MCP `list_repos` / `detect_changes` report "repository not found".
- After ensure, pass the **absolute worktree path** as `repo` on GitNexus MCP/CLI calls. The bare repository name is ambiguous when several checkouts are registered.
- Do not rebuild the graph while files are being modified.
- Only rebuild on a clean working tree. `ensure_gitnexus.py` shares the primary `.gitnexus/` via a junction/symlink; do not `gitnexus clean` from a worktree that shares that index.
- Use the graph as a snapshot, not a live source of truth.
- Query the graph first, then read files directly.

## Workflow

1. (Preamble) Ensure you're in a dedicated local branch/worktree under `.worktree/<session-name>` and activate the project environment (see [docs/commands.md](docs/commands.md)). **Make sure the branch is up to date**. Read the `docs/insights.md` file and the [Workflow Rules](#workflow-rules).
2. (GitNexus) From this checkout run `python scripts/ensure_gitnexus.py`, then read the [GitNexus](#gitnexus--code-intelligence) section. Confirm `npx gitnexus list` shows **this** worktree path before using MCP tools.
...
GitNexus keys `~/.gitnexus/registry.json` by checkout path. A linked worktree is invisible until that path is registered.

- Run `python scripts/ensure_gitnexus.py` from the current checkout at session start (`--all` registers every linked worktree).
- If the worktree has no `.gitnexus/`, the script junctions/symlinks the primary checkout's index and runs `npx gitnexus index`. It only runs `npx gitnexus analyze --index-only` when no sibling index exists.
- After ensure, pass the absolute worktree path as `--repo` / MCP `repo`. Do not `gitnexus clean` from a worktree that shares the primary index.
